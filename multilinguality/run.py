        ...
        for batch in dataloader:
            ...
            if global_step % eval_period == 0:
                ...
                if "Bert" in model_class:
                    if prompt_tune:
                        keys = ["bert.embeddings.word_embeddings.new_embed.weight"]
                        model_state_dict = {"bert.embeddings.word_embeddings.new_embed.weight": model.state_dict()["bert.embeddings.word_embeddings.new_embed.weight"]}

                    elif head_tune:
                        keys = ["pooler.dense.weight"]
                        model_state_dict = {key: model.state_dict()[key if n_gpus==1 else ""+key].cpu() for key in keys}
                    elif transform_tune:
                        keys = ["cls.predictions.transform.dense.weight"]
                        model_state_dict = {key: model.state_dict()[key if n_gpus==1 else ""+key].cpu() for key in keys}
                    else:
