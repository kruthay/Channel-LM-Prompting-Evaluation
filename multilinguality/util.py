...
def prepend_task_tokens(tokenizer, inputs, n_prefix):
    ...
    if task_token_ids.shape[-1] != n_prefix:
        input_ids_without_special_tokens = []
        for sublist in task_token_ids:
            sublist_without_special_tokens = sublist[1:-1]
            input_ids_without_special_tokens.append(sublist_without_special_tokens)
        task_token_ids = torch.stack(input_ids_without_special_tokens)
    
    print(task_token_ids)
    print(task_token_ids.shape)
    print("Edit Pllied")
    print(n_prefix)
