...
access_token = "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"
...
def load_checkpoint(gpt2, checkpoint=None,
    ...
    if gpt2  == 'german-gpt2':
        model = AutoModelForCausalLM.from_pretrained("dbmdz/german-gpt2", use_auth_token=access_token)
    elif 'bert' in gpt2  :
        model = AutoModelForCausalLM.from_pretrained(gpt2, use_auth_token=access_token)
    else:
    ...
    if checkpoint is not None:
        ...
        if "Bert" in model_class:
            if prompt_tune:
                ...
                weight = torch.load(checkpoint)["bert.embeddings.word_embeddings.new_embed.weight"]
                model.bert.embeddings.word_embeddings.new_embed._load_from_state_dict({"weight": weight}, "", None, True, [], [], "")
            elif head_tune:
                weight = torch.load(checkpoint)["pooler.dense.weight"]
                set_separate_lm_head(model, mapping=mapping)
                model.pooler.dense._load_from_state_dict({"weight": weight}, "", None, True, [], [], "")
            elif transform_tune:
                weight = torch.load(checkpoint)["cls.predictions.transform.dense.weight"]
                set_transformed_lm_head(model)
                model.cls.predictions.transform.dense._load_from_state_dict({"weight": weight}, "", None, True, [], [], "")
        else:
            if prompt_tune:
                set_extra_embeddings(model, n_prefix=n_prefix)
...
def set_extra_embeddings(model, n_prefix):
    model_class = model.__class__.__name__
    if "Bert" in model_class:
        model.bert.embeddings.word_embeddings = MyEmbedding(model.bert.embeddings.word_embeddings, n_prefix)
    else:
...
def set_separate_lm_head(model, mapping):
    model_class = model.__class__.__name__
    if "Bert" in model_class:
        model.bert.set_output_embeddings(
            MyLMHead(model.bert.lm_head, mapping))
    else:
...
def set_separate_embeddings(model, mapping):
    model_class = model.__class__.__name__
    if "Bert" in model_class:
        model.bert.embeddings.word_embeddings = MyEmbedding2(model.bert.embeddings.word_embeddings, mapping)
    else:
...
def set_transformed_lm_head(model):
    ...
    if "Bert" in model_class:
        model.bert.set_output_embeddings(
            MyLMHeadWithTransform(model.bert.lm_head))
    else:
        model.set_output_embeddings(
